...
classifySingleLeptonEvent :: PhyEventClassified -> Maybe EventType
classifySingleLeptonEvent ev@PhyEventClassified {..} = do 
...
main = do 
  ...
  bstr <- LB.readFile "ADMXQLD311MST500.0_stoppair_full_LHC7ATLAS_NoMatch_NoCut_Cone0.4_Set1_pgs_events.lhco.gz"
  ...
  -- print $ map meff signalevts
      onelepevts = mapMaybe classifySingleLeptonEvent signalevts 
  -- print (length evts) 
    
  mapM_ print onelepevts
